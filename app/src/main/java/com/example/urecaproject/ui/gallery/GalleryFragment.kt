...
class GalleryFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val adapter = GalleryRecyclerAdapter(context!!, R.layout.gallery_item, images)
        galleryRv.adapter = adapter
        galleryRv.layoutManager = GridLayoutManager(context, 3)

        galleryViewModel.getImageList().observe(viewLifecycleOwner, Observer<List<String>> { listOfImage ->
